...
int main(int argc, char **argv)
{
    ...
    if (!parsecmdline("src_rgb_2_conrat", "Transform from RGB to (L, C1, C2) - Conrat coordinates", argc, argv, options, parameters))
        return EXIT_FAILURE;
    ...
    libUSTG::fiRgb2Conrat(input.v(0), input.v(1), input.v(2), L.v(), C1.v(), C2.v(), input.w() * input.h());
