...
int main(int argc, char **argv)
{
	...
	if (!parsecmdline("src_filter_patch_median", "Filters patches by median", argc, argv, options, parameters))
		return 0;
    ...
    for (int i=0; i < nIter; i++)
    {
        ...
        {
            libUSTG::fiPatchMedian(input.v(iC), output.v(iC), fRadius, input.w(), input.h());
