	...
	for(int h=0;h<hs_channels;h++)
	{
		...
		for(int x = spat_patch_step; x < height_ms_new-spat_patch_step; x++)
		{
			...
			for(int y = spat_patch_step; y < width_ms_new-spat_patch_step; y++)
			{
				...
				/*if(l_comp==15862)
				{
					for(int w=0; w<tmp_wxy.size();w++)
					{
						float weight = libUSTG::wxSLUT(tmp_wxy[w], lut);
						std::cout<<"h= "<<h<<", tmp_wxy= "<<tmp_wxy[w]<<", weight= "<<weight<<std::endl;
					}
					...
					/*if(l_comp==15862)
                                        {
                                                ...
                                                {
                                                        float weight = libUSTG::wxSLUT(tmp_wxy[w], lut);
                                                        std::cout<<"h= "<<h<<", tmp_wxy.size= "<<tmp_wxy.size()<<", tmp_pxy.size= "<<tmp_pxy.size()<<", tmp_wxy= "<<tmp_wxy[w]<<", tmp_pxy= "<<tmp_pxy[w]<<std::endl;
                                                }
