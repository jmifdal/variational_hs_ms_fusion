                           ...
                           float tol, int maxIter, int sampling, float stdBlur, int hs_channels, int ms_channels, int width, int height)
    {
        ...
        while(iter < maxIter && error >= tol)
        {
            ...
            for(int h = 0; h < hs_channels; h++)
            {
                ...
                //The part corresponding to the L1 norm
                for(int i = 0; i < dim; i++)
                {
                    ...
                    if(Phintval > fTiny)
                    {
                        ...
                        } else
                        {
                            ...
                            constraint = (PH[h][i] * fHint[h][i]) / Phintval;
                        }
                    ...
                    /////!!!! Compute value of each part of the primal variable
                    printf("NL = %f,  MS data = %f,  HS data = %f, PAN constraint = %f\n", nldiv[h][i], aux[i], aux2[i], constraint);
                }
